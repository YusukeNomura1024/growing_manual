    ...
    columns do
      column do
        panel "問い合わせ一覧(#{Message.contact.not_reply.count}件)" do
          ul class: "scrollbars" do
            Message.preload(:user, :review, :manual).contact.not_reply.map do |message|
      ...
      column do
        panel "違反報告一覧(#{Message.violation_report.not_reply.count}件)" do
          ul class: "scrollbars" do
            Message.preload(:user, :review, :manual, [review: :user]).violation_report.not_reply.map do |message|
